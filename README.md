### Important need to place jdk 1.8 tar.gz file and apache 1.9 tar.gz in project directory in order to perform contanerization with docker swarm service using Docker file (place jdk1.8 and apache tomcat 1.9 version tar.gz file in project directory)

#### In this project java springboot web application called DevopsBloggingSite is up and running using DevOps culture and tools with maven
#### maven is used to build java sprinboot application
#### Jenkins pipeline are used to maintain continous Integration (CI)
#### Jfrog is used to store artifacts (war files)
#### SonarQube is used to check test coverage and bugs and vulnerabilities to maintain continous testing
#### Docker swarm service is used for contanerization and continous Deployment (CD) to build image and create docker service using ansible playbook 
#### Apache tomcat server is used to deploy Devops blogging web application on Browser.
